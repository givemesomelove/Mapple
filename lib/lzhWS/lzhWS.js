...
class LzhWS {
    constructor() {
        ...
        this.addRouterMap = this.addRouterMap.bind(this);
        this.on = this.on.bind(this);
        this.sendMessage2Client = this.sendMessage2Client.bind(this);
        this.sendMessage2Room = this.sendMessage2Room.bind(this);
        this.sendMessage2World = this.sendMessage2World.bind(this);
    ...
    addRouterMap(socket) {
        socket.on("disconnect", () => {
            console.log(`用户${socket.id}断开了${this.space.name}房间`);
        });
        ...
        this.routers.forEach((handler, path) => {
            socket.on(path, async (data, block) => {
                try {
                    ...
                    block && block({ code: 0, data: result });
                } catch (err) {
                    block && block({ code: 1, message: err.message });
                }
            });
        });
    }
